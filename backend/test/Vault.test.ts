...
describe("Vault.sol – OneToken", function () {
  async function deployVaultFixture() {
    ...
    const usdc = (await MockUSDCFactory.deploy("Mock USDC", "mUSDC", 6)) as MockUSDC;
    await usdc.waitForDeployment();

    // Mint USDC for users
    await (await usdc.mint(owner.address, ethers.parseUnits("10000", 6))).wait();
    ...
    const allocation = [{ token: await usdc.getAddress(), weight: ethers.parseUnits("1", 18), active: true }];
  ...
  it("permet à un utilisateur de déposer des USDC et de recevoir des parts (shares)", async function () {
    ...
    const tx = await vault.connect(user)["deposit(uint256,address,string)"](amount, user.address, "equilibree");
    await expect(tx)
    ...
    expect(await usdc.balanceOf(user.address)).to.eq(ethers.parseUnits("9000", 6));
  });
  ...
  it("permet à un utilisateur de retirer (redeem) et applique le nonReentrant", async function () {
    ...
    await (await usdc.connect(user).approve(await vault.getAddress(), depositAmount)).wait();
    await vault.connect(user)["deposit(uint256,address,string)"](depositAmount, user.address, "equilibree");
    ...
    expect(await usdc.balanceOf(user.address)).to.eq(ethers.parseUnits("10000", 6));
  });
  ...
  it("le Vault ne détient JAMAIS de tokens RWA (invariant)", async function () {
    ...
    expect(await vault.asset()).to.not.eq("0x0000000000000000000000000000000000000000");
  });
  ...
  it("pausable : seul l'owner peut pauser et unpause, et bloque les dépôts/retraits", async function () {
    ...
    await (await usdc.connect(user).approve(await vault.getAddress(), amount)).wait();
    await expect(
      vault.connect(user)["deposit(uint256,address,string)"](amount, user.address, "equilibree")
