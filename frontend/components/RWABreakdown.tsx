...
const RWABreakdown: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      if (!totalAssets || !allocations.length || !registeredTokens.length) return;
      ...
      const promises = allocations
        .filter(a => a.active)
        .map(async (alloc) => {
          const tokenInfo = registeredTokens.find(t => t.tokenAddress.toLowerCase() === alloc.token.toLowerCase());
      ...
      <ul>
        {breakdown.map((item) => (
          <li key={item.symbol} style={{ marginBottom: 8 }}>
            <strong>{item.symbol}</strong> : {item.percent.toFixed(2)}% — {item.value.toLocaleString('fr-FR', { maximumFractionDigits: 2 })} USDC
          </li>
        ))}
      </ul>
    </div>
